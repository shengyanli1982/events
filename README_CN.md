...
`Events` 的重点是注册事件处理函数和发射事件，而将函数的执行留给您来决定。您可以使用 [`karta`](https://github.com/shengyanli1982/karta) 在单独的任务中执行这些函数，因为它实现了 `Pipeline` 接口。

实现 `Pipeline` 接口来处理事件，并在您的应用程序中充分利用 `Events` 的强大功能。
