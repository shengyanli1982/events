...
`NewSimpleEventEmitter` 方法是 events 项目中的一个鲜为人知的特性，位于 `/contrib/lazy` 目录中。这个方法允许创建一个带有各种预设默认值的 `EventEmitter` 对象。使用这个方法创建的 `EventEmitter` 的行为与使用 `NewEventEmitter` 方法创建的一样。
